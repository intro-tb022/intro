...
Se tiene un set de archivos `csv` con informacion variada de todos los Pokemones existentes. Descargar [aquí](tp2/data.zip).

Estos archivos `.csv` deben ser cargados al comienzo de la ejecución del programa y mantener su estado en memoria RAM. Se deben procesar de manera eficiente. Tener en cuenta que tienen cientos de miles de líneas y de lo contrario puede hacer que la experiencia con el sistema sea deplorable.
